...
public class PhotoUtils {
    ...
    public PhotoUtils(int width, int height, String crop, OnPhotoResultListener onPhotoResultListener) {
        this.width = width;
        this.height = height;
        this.crop = crop;
...
//            return Uri.parse(App.cropDir  + CROP_FILE_NAME);
//        } else {
//            return Uri.parse(App.cropDir  + CROP_FILE_NAME);
//        }

        if (CommonUtils.checkSDCard()) {
            return Uri.fromFile(Environment.getExternalStorageDirectory()).buildUpon().appendPath(BuildConfig.APPLICATION_ID).appendPath("xchat").appendPath("ImageCrop").appendPath(CROP_FILE_NAME).build();
        } else {
            return Uri.fromFile(activity.getCacheDir()).buildUpon().appendPath(BuildConfig.APPLICATION_ID).appendPath("xchat").appendPath("ImageCrop").appendPath(CROP_FILE_NAME).build();
        }
    }
