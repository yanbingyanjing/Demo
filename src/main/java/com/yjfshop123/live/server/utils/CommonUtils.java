...
package com.yjfshop123.live.server.utils;
...
public class CommonUtils {
	...
	public static Bitmap create2DCode(String str) {
		try {
			...
			BitMatrix matrix = new MultiFormatWriter().encode(str, BarcodeFormat.QR_CODE, 1600, 1600);
     ...
     * @return
     */
    public static Bitmap create2DCodeWithWhite(String str) {
        try {
            ...
            // 生成二维矩阵,编码时指定大小,不要生成了图片以后再进行缩放,这样会模糊导致识别失败
            BitMatrix matrix = new MultiFormatWriter().encode(str, BarcodeFormat.QR_CODE, 1600, 1600,hints);
           // matrix = deleteWhite(matrix);//删除白边
            int width = matrix.getWidth();
            int height = matrix.getHeight();
