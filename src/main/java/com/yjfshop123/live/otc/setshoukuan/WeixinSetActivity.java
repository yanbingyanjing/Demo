...
public class WeixinSetActivity extends BaseActivityH implements CosXmlUtils.OSSResultListener {
        ...
        if (getIntent() != null) {
            ...
            if (!TextUtils.isEmpty(getIntent().getStringExtra("data"))) {
                ...
                if (!TextUtils.isEmpty(data.qrcode_url)) {
                    ...
                    Glide.with(WeixinSetActivity.this).load(qrcode_url).into(erweima);
                }
            }
            delete.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if (!TextUtils.isEmpty(select_id) && !TextUtils.isEmpty(id) && select_id.equals(id)) {
                        NToast.longToast(WeixinSetActivity.this,"默认收款方式，请勿删除，如需删除，请重新选择默认收款方式后再进行此操作！");
                        return;
                    }
                    DialogUitl.showSimpleHintDialog(WeixinSetActivity.this, getString(R.string.prompt), getString(R.string.ac_select_friend_sure), getString(R.string.cancel),
                            "确认删除收款方式？", true, true,
    ...
    public void delete() {
        ...
        OKHttpUtils.getInstance().getRequest("app/trade/delCard", body, new RequestCallback() {
            ...
            public void onError(int errCode, String errInfo) {
                LoadDialog.dismiss(WeixinSetActivity.this);
                NToast.shortToast(WeixinSetActivity.this, errInfo);
            }

            @Override
            public void onSuccess(String result) {
                LoadDialog.dismiss(WeixinSetActivity.this);
                NToast.shortToast(WeixinSetActivity.this, "删除成功");
                finish();
            }
        });
    ...
    public void fix() {
        ...
        OKHttpUtils.getInstance().getRequest("app/trade/editCard", body, new RequestCallback() {
            ...
            public void onSuccess(String result) {
                LoadDialog.dismiss(WeixinSetActivity.this);
                NToast.shortToast(WeixinSetActivity.this, "修改成功");
                finish();
            }
        });
    ...
    public void confiry() {
        ...
        OKHttpUtils.getInstance().getRequest("app/trade/addCard", body, new RequestCallback() {
            ...
            public void onSuccess(String result) {
                LoadDialog.dismiss(WeixinSetActivity.this);
                NToast.shortToast(WeixinSetActivity.this, "添加成功");
                finish();
            }
        });
...
//                break;
            ...
            case 1:
                ...
                if (data != null) {
                    ...
                    tips.setVisibility(View.GONE);
                    erweima.setVisibility(View.VISIBLE);
                    Glide.with(WeixinSetActivity.this).load(selectPic).into(erweima);
