      ...
      /*  if (!isFromRefresh)
        ...
        OKHttpUtils.getInstance().getRequest("app/trade/otcOrderDetail", body, new RequestCallback() {
            ...
            public void onError(int errCode, String errInfo) {
                ...
                NToast.shortToast(OrderDetailActivity.this, errInfo);
            }
                ...
                if (otcOrderResponse != null) {
                    ...
                    if (!TextUtils.isEmpty(otcOrderResponse.pay_type)) {
                        if (otcOrderResponse.pay_type.equals("card")) {
                            ...
                            Glide.with(OrderDetailActivity.this).load(R.mipmap.bank).into(payTypeLogo);
                        }
                        if (otcOrderResponse.pay_type.equals("alipay")) {
                            payType.setText(getString(R.string.tixian_ali));
                            Glide.with(OrderDetailActivity.this).load(R.mipmap.zhifubao).into(payTypeLogo);
                        }
                        if (otcOrderResponse.pay_type.equals("wechat")) {
                            payType.setText(getString(R.string.setting_wx));
                            Glide.with(OrderDetailActivity.this).load(R.mipmap.weixin).into(payTypeLogo);
                        }
                    }
                    if (otcOrderResponse.status != 1)
                        return;
    ...
    public void onViewClicked(View view) {
        switch (view.getId()) {
            case R.id.text_right:
                LoadDialog.show(OrderDetailActivity.this);
                String body = "";
                try {
                    body = new JsonBuilder()
                ...
                OKHttpUtils.getInstance().getRequest("app/user/getUserHomeByUid", body, new RequestCallback() {
                    @Override
                    public void onError(int errCode, String errInfo) {
                        LoadDialog.dismiss(OrderDetailActivity.this);NToast.shortToast(mContext,errInfo);
                    }
...
//                DialogUitl.showSimpleDialog(this, "确定取消交易？", new DialogUitl.SimpleCallback() {
//                    @Override
...
//                });
    ...
    public void cancelbuy() {
        ...
        OKHttpUtils.getInstance().getRequest("app/trade/payCancel", body, new RequestCallback() {
            ...
            public void onSuccess(String result) {
                LoadDialog.dismiss(OrderDetailActivity.this);
                NToast.shortToast(OrderDetailActivity.this, "取消成功");
                finish();
            }
        });
