...
import com.yjfshop123.live.live.live.common.widget.gift.utils.LiveIconUtil;
...
public class LiveLinkMicPkViewHolder extends AbsViewHolder {
    ...
    @Override
    public void init() {
        mHalfScreenWidth = getScreenWdith() / 2;
...
//        mAnimator2 = ValueAnimator.ofFloat(0, 18);
        mAnimator2 = ValueAnimator.ofFloat(0, 49);
...
//        mAnimator2.setDuration(800);
        mAnimator2.setDuration(3200);
        mAnimator2.setInterpolator(new LinearInterpolator());

        mTime = (TextView) findViewById(R.id.time);
        mResultImageView = (ImageView) findViewById(R.id.result);
        mOffsetX = CommonUtils.dip2px(mContext, 70) / 2;
        mOffsetY = CommonUtils.dip2px(mContext, 41) / 2;
    ...
    public void onEnterRoomPkStart() {
        if (mFrameImageView != null) {
            mFrameImageView.setImageResource(R.mipmap.icon_live_vs);
        }
        if (mPkProgressBar != null && mPkProgressBar.getVisibility() != View.VISIBLE) {
            mPkProgressBar.setVisibility(View.VISIBLE);
        }
    ...
    private ValueAnimator getEndValueAnimator(int result) {
        ...
        if (result == 0) {
//            valueAnimator = ObjectAnimator.ofFloat(mResultImageView, "translationY", CommonUtils.dip2px(mContext, 80));
            valueAnimator = ObjectAnimator.ofFloat(mResultImageView, "translationY", CommonUtils.dip2px(mContext, 52));
        } else {
