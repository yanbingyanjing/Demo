...
public class LiveGiftViewHolder extends AbsViewHolder {
    ...
    public void init() {
        ...
        mDp214 = CommonUtils.dip2px(mContext, 214);
        mHandler = new Handler() {
            ...
            public void handleMessage(Message msg) {
                ...
                if (mGiftCount != null && mGiftCount.getVisibility() != View.VISIBLE) {
                    ...
                    mGiftCount.setText(LiveTextRender.renderGiftCount(mContext, mLianCount));
                }
                if (mGiftCount != null) {
...
//            ImgLoader.display(bean.getGiftIcon(), mGiftIcon);
            if(!isDestroy((Activity)mContext)){
                Glide.with(mContext)
                        .load(bean.getLiveReceiveGiftBeanItem().getGiftIcon())
                        .into(mGiftIcon);
            }
            mContent.setText(LiveTextRender.renderGiftInfo2(mContext, bean.getLiveReceiveGiftBeanItem().getGiftName()));
            if (bean.getLiveReceiveGiftBeanItem().getGiftCount() > 1) {
                mGiftGroupCount.setText("x" + bean.getLiveReceiveGiftBeanItem().getGiftCount());
