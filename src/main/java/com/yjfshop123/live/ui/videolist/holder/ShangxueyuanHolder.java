...
public class ShangxueyuanHolder extends RecyclerView.ViewHolder {
...
//    TextView share_url;
    ...
    public ShangxueyuanHolder(View itemView, int screenWidth) {
        super(itemView);
...
//            }
//        });
        time.setText(iItem.create_time);
...
//        }
        ...
        if (TextUtils.isEmpty(iItem.content)) {
            ...
            SystemUtils.toggleEllipsize(itemView.getContext(), community_item_content,
                    ...
                    R.color.color_2D9DD8, false);
            community_item_content.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    if(!TextUtils.isEmpty(community_item_content.getText().toString())&&iItem.content.length()>91){
                        if(community_item_content.getText().toString().endsWith("...全文")){
                            SystemUtils.toggleEllipsize(itemView.getContext(), community_item_content,
                                    94, "内容：" + PandaEmoTranslator
                                            .getInstance()
                                            .makeEmojiSpannable(iItem.content).toString(), itemView.getContext().getString(R.string.all__wen),
                                    R.color.color_2D9DD8, true);
                        }else {
                            SystemUtils.toggleEllipsize(itemView.getContext(), community_item_content,
                                    94, "内容：" + PandaEmoTranslator
                                            .getInstance()
