...
public class VoisePlayingIcon extends View {
    ...
    public VoisePlayingIcon(Context context, @Nullable AttributeSet attrs) {
        ...
        pointerColor = ta.getColor(R.styleable.voisePlayingIconAttr_pointer_color, Color.WHITE);
        pointerNum = ta.getInt(R.styleable.voisePlayingIconAttr_pointer_num, 4);//指针的数量，默认为4
        pointerWidth = DensityUtils.dp2px(getContext(),
                ta.getFloat(R.styleable.voisePlayingIconAttr_pointer_width, 1.6f));//指针的宽度，默认5dp
     ...
     */
    public void start() {
        NLog.d("开始波动","0"+isPlaying);
        if (!isPlaying) {
            NLog.d("开始波动","1"+isPlaying);
